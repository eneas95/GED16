...
mse3
```
  Multiple R-squared (MSR):  0.8072
  
```{r reg_summary4}
summary(abalone_rls4)
```
  Multiple R-squared:  0.3097
...
```
  ...
  - Par `length`-`shell_w`:   R-squared:  0.8071
  - Par `length`-`rings`:     R-squared:  0.3095
...
```{r corr2}
#M
cor(abalone_m$length, abalone_m$diameter)
#F
cor(abalone_f$length, abalone_f$diameter)
#I
cor(abalone_i$length, abalone_i$diameter)
```
...
```{r modred_m}
# Constrói modelo de regressao linear simples (rls)
abalone_rls_m <- lm(length ~ diameter, data = abalone_m)
# variável de resposta: `lenght` 
# variável explicativa: `diameter`
...
```
  ...
  **Modelos Ajustados**
  ...
  2. Para `sex` igual a `F`:
  
  3. Para `sex` igual a `I`:
